...
const VISION_MODEL =
	process.env.LLM_VISION_MODEL || "anthropic/claude-4.5-haiku";
...
function toProviderModelId(modelId: string): ProviderModelId {
	...
	throw new Error(
		`Unsupported LLM provider in "${modelId}". Use anthropic/* or google/*.`,
	);
}
...
function getModel(modelId: string) {
	...
	try {
		...
		throw new Error(
			`Unsupported LLM model "${modelId}". Expected format "provider/model".`,
		);
	}
}
	...
	if (hasTextRedactions || hasVisualRedactions) {
		...
		if (hasTextRedactions) {
			...
			for (const r of redactions!) {
				...
				const nodeInfo =
					r.nodeIds.length > 0 ? ` in nodes [${r.nodeIds.join(", ")}]` : "";
...
export async function triageQuery(
	...
	const messages: ModelMessage[] = [
		{
			role: "system",
			content: getTriagePrompt(autoScreenshot, deviceInfo, languageCode),
		},
...
async function* streamChatCompletion(
	...
	ensureProviderKey(model);
	console.log(
		`[LLM] 🤖 Streaming ${model} with ${messages.length} messages...`,
	);
...
export async function* streamVisualAnalysis(
	...
	const messages: ModelMessage[] = [
		{
			role: "system",
			content: getVisualAnalysisPrompt(
				autoScreenshot,
				deviceInfo,
				languageCode,
			),
...
export async function* streamTextAnalysis(
	...
	const messages: ModelMessage[] = [
		{
			role: "system",
			content: getTextAnalysisPrompt(autoScreenshot, deviceInfo, languageCode),
		},
		...conversationHistory.map((h) => ({
			role: h.role,
			content: h.content,
