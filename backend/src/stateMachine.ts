...
import { streamTextToSpeech, transcribeAudio } from "./services/elevenlabs.js";
import {
	streamTextAnalysis,
...
interface PendingScreenshotRequest {
	...
	bufferedScreenshotReceivedAt?: string;
}
 ...
 */
function getSessionDeviceInfo(session: Session): {
	manufacturer: string;
	model: string;
...
export async function handleAudioReceived(
	...
	if (duration < MIN_AUDIO_DURATION_S) {
		console.log(
			`[${session.id}] Audio too short (${duration.toFixed(2)}s < ${MIN_AUDIO_DURATION_S}s), discarding`,
		);
	...
	if (session.autoScreenshot) {
		...
		console.log(
			`[${session.id}] Auto-screenshot ON — pending entry created before STT`,
		);
	}
	...
	try {
		// Step 1: STT
		console.log(
			`[${session.id}] Step 1: Transcribing audio (${audioBuffer.length} bytes)...`,
		);
		...
		// Turn IDs are allocated from persisted conversation data so restarts
		// and concurrent requests cannot reuse IDs.
		const turnId = await startTurn(session.clientId, session.conversationId, {
			autoScreenshot: session.autoScreenshot,
			audioReceivedAt,
		});
		if (!turnId) {
			console.error(
				`[${session.id}] Failed to allocate turn ID for conversation ${session.conversationId}`,
			);
			sendError(ws, "Could not start a new turn. Please try again.");
			return;
		}
		session.turnCounter = Math.max(session.turnCounter, turnId);
		session.currentTurnId = turnId;

		console.log(`\n${"=".repeat(60)}`);
		console.log(
			`[${session.id}] Starting audio processing pipeline (turn #${turnId}, conv=${session.conversationId})`,
		);
		console.log(`${"=".repeat(60)}\n`);

		// Save raw audio to disk (fire and forget)
		saveAudioInput(
			session.clientId,
			session.conversationId,
			turnId,
			...
			.then((file) =>
				updateTurn(session.clientId, session.conversationId, turnId, {
					input: { audioFile: file },
				}),
			)
			.catch((err) =>
				console.error(`[DataStore] Failed to save audio input:`, err),
			);
		...
		if (session.autoScreenshot) {
			...
			if (pending) {
				...
				if (pending.bufferedScreenshot) {
					...
					console.log(
						`[${session.id}] Buffered screenshot found, running visual analysis...`,
					);
					clearTimeout(pending.timer);
					...
					if (msg.redacted) {
						console.log(
							`[${session.id}] Screenshot has PII redactions: ${msg.redactions?.length ?? 0} types`,
						);
					}
					...
					saveTiming(session.clientId, session.conversationId, turnId, {
						timing: {
							screenshotReceivedAt:
								pending.bufferedScreenshotReceivedAt ??
					...
					Promise.all([
						...
						.catch((err) =>
							console.error(
								`[DataStore] Failed to save screenshot/UI tree:`,
								err,
							),
						);

					saveTiming(session.clientId, session.conversationId, turnId, {
						timing: { llmStartedAt: new Date().toISOString() },
					});
		...
		saveTiming(session.clientId, session.conversationId, turnId, {
			timing: { triageStartedAt: new Date().toISOString() },
		});
		const startTriage = Date.now();
		const triage = await triageQuery(
			transcript,
			getHistoryForLLM(session),
			signal,
			session.autoScreenshot,
		...
		console.log(
			`[${session.id}] Triage result: needsScreenshot=${triage.needsScreenshot}, reason="${triage.reason}"`,
		);

		// Save triage result + timing to disk
		saveTiming(session.clientId, session.conversationId, turnId, {
			triage: {
				needsScreenshot: triage.needsScreenshot,
		...
		if (triage.needsScreenshot) {
			...
			const timer = setTimeout(() => {
				if (pendingScreenshots.has(session.id)) {
					pendingScreenshots.delete(session.id);
					console.warn(
						`[${session.id}] Screenshot request timed out after 60s`,
					);
			...
			console.log(
				`[${session.id}] Triage needs screenshot — TTS-streaming spoken request`,
			);
			saveTiming(session.clientId, session.conversationId, turnId, {
				timing: { screenshotRequestedAt: new Date().toISOString() },
			});

			// TTS the spoken request and stream as answer_start → PCM → answer_end
			const spokenText = stripAudioTags(
				triage.spokenRequest ||
					"Let me take a look at your screen to help you with that.",
			);
			const confirmLabel = triage.confirmLabel || "Share screen";

			sendJSON(ws, { type: "answer_start" });
			try {
				for await (const pcmChunk of streamTextToSpeech(
					spokenText,
					session.languageCode,
					signal,
				)) {
			...
			console.log(
				`[${session.id}] Waiting for screenshot response (confirmLabel="${confirmLabel}")...\n`,
			);
			return;
		}
...
export async function handleScreenshotResponse(
	...
	if (!pending.userText) {
		...
		pending.bufferedScreenshotReceivedAt = new Date().toISOString();
		return;
	}
	...
	Promise.all([
		...
		.catch((err) =>
			console.error(`[DataStore] Failed to save screenshot/UI tree:`, err),
		);

	try {
		if (message.redacted) {
			console.log(
				`[${session.id}] Screenshot has PII redactions: ${message.redactions?.length ?? 0} types`,
			);
		}
		console.log(
			`[${session.id}] Running streaming visual analysis with screenshot...`,
		);
		saveTiming(session.clientId, session.conversationId, turnId, {
			timing: { llmStartedAt: new Date().toISOString() },
		});
		...
		sendError(
			ws,
			"Something went wrong analyzing the screenshot. Please try again.",
		);
	}
}
...
export async function handleScreenshotDeclined(
	...
	try {
		console.log(
			`[${session.id}] Screenshot declined, falling back to streaming text analysis...`,
		);
		const emptyUiTree: UiTree = {
			screen: { packageName: "unknown", timestamp: Date.now() },
			nodes: [],
...
/**
 * Resolve highlight bounds from the UI tree. Uses the snapshot that was analyzed,
 * so elementIds match. Avoids client-side re-traversal which can assign different IDs.
 */
function resolveHighlightBounds(
	highlights: Array<{ elementId: string; label: string }>,
	uiTree: UiTree,
): Array<{
	elementId: string;
	label: string;
	...
	const nodeMap = new Map(uiTree.nodes.map((n) => [n.id, n]));
	return highlights.map((h) => {
		const node = nodeMap.get(h.elementId);
		if (node?.bounds) {
			return { ...h, bounds: node.bounds };
		}
		return h;
	});
}
 ...
 * When uiTree is provided (from screenshot analysis), highlights are enriched with
 * bounds so the client can display them without re-traversing the tree.
 */
		...
		if (!streamingStarted) {
			if (!firstAudioRecorded) {
				...
				saveTiming(session.clientId, convId, turnId, {
					timing: { ttsFirstAudioAt: new Date().toISOString() },
				});
			}
			sendJSON(ws, { type: "answer_start" });
	...
	try {
		...
		for await (const chunk of analysisGenerator) {
			...
			if (!firstTokenRecorded) {
				...
				saveTiming(session.clientId, convId, turnId, {
					timing: { llmFirstTokenAt: new Date().toISOString() },
				});
			}
			...
			for (const sentence of sentences) {
				...
				sentenceCount++;
				console.log(
					`[${session.id}] Streaming TTS for sentence ${sentenceCount}: "${clean.substring(0, 50)}..."`,
				);
		...
		// LLM streaming done
		saveTiming(session.clientId, convId, turnId, {
			timing: { llmCompletedAt: new Date().toISOString() },
		});
		...
		if (remaining) {
			...
			if (clean) {
				sentenceCount++;
				console.log(
					`[${session.id}] Streaming TTS for final fragment: "${clean.substring(0, 50)}..."`,
				);
				for await (const pcmChunk of streamTextToSpeech(
					clean,
					session.languageCode,
		...
		if (pcmChunks.length > 0) {
			saveAudioOutput(session.clientId, convId, turnId, pcmChunks)
				.then((file) =>
					updateTurn(session.clientId, convId, turnId, {
						output: { audioFile: file },
					}),
				)
				.catch((err) =>
					console.error(`[DataStore] Failed to save audio output:`, err),
				);
		}
		...
		if (nextStep) {
			const timer = setTimeout(() => {
				if (pendingScreenshots.has(session.id)) {
					pendingScreenshots.delete(session.id);
					console.warn(
						`[${session.id}] Next-step screenshot timed out after 60s`,
					);
				}
			}, 60_000);
		...
		const highlightsToSend = uiTree
			? resolveHighlightBounds(highlights, uiTree)
			: highlights;
		...
		console.log(
			`[${session.id}] Streaming complete! ${sentenceCount} sentences sent${nextStep ? ` (nextStep, label="${confirmLabel}")` : ""}`,
		);
	} catch (err) {
		if (
			(err as Error).name === "AbortError" ||
