...
export interface TurnData {
	...
	screenshot?: {
		file?: string;
		uiTreeFile?: string;
		/** Additional screenshots from next-step flows (same turn, multiple captures) */
		files?: string[];
		uiTreeFiles?: string[];
	};
...
function isoToMs(v?: string): number | null {
	if (!v) return null;
	const t = Date.parse(v);
	return Number.isFinite(t) ? t : null;
}

function turnTimestampForTieBreak(turn: TurnData): number {
	...
	const candidates = [
		...
		tm.llmFirstTokenAt,
		tm.llmStartedAt,
		tm.screenshotReceivedAt,
		tm.triageCompletedAt,
		tm.sttCompletedAt,
		tm.sttStartedAt,
		tm.audioReceivedAt,
	];
...
function turnCompletenessScore(turn: TurnData): number {
	let score = 0;
	if (turn.input.transcript) score += 8;
	if (turn.input.audioFile) score += 2;
	if (turn.output.text) score += 8;
	if (turn.output.audioFile) score += 2;
	if (turn.output.highlights?.length) score += 2;
	if (turn.screenshot?.file) score += 2;
	if (turn.screenshot?.uiTreeFile) score += 2;
	if (turn.triage) score += 2;
	for (const v of Object.values(turn.timing ?? {})) {
		if (v != null) score += 1;
	}
	return score;
}

function dedupeTurnsForRead(turns: TurnData[]): TurnData[] {
	const byTurnId = new Map<number, TurnData>();
	for (const turn of turns) {
		...
		const candidateScore = turnCompletenessScore(turn);
		if (candidateScore > existingScore) {
			byTurnId.set(turn.turnId, turn);
			continue;
		}
		if (candidateScore === existingScore) {
			const existingTs = turnTimestampForTieBreak(existing);
			const candidateTs = turnTimestampForTieBreak(turn);
			if (candidateTs >= existingTs) {
				byTurnId.set(turn.turnId, turn);
			}
		}
	}
	return Array.from(byTurnId.values()).sort((a, b) => a.turnId - b.turnId);
}
...
export function startTurn(
	...
	return withWriteLock(key, async () => {
		...
		let maxTurnId = 0;
		for (const existingTurn of conv.turns) {
			if (
				typeof existingTurn.turnId === "number" &&
				existingTurn.turnId > maxTurnId
			) {
				maxTurnId = existingTurn.turnId;
			}
		}
		allocatedTurnId = maxTurnId + 1;
		const turn: TurnData = {
			turnId: allocatedTurnId,
...
export function updateTurn(
	...
	return withWriteLock(key, async () => {
		const conv = await readConversationRaw(clientId, sessionId);
		if (!conv) return;
		let turn: TurnData | undefined;
		for (let i = conv.turns.length - 1; i >= 0; i--) {
			if (conv.turns[i].turnId === turnId) {
				turn = conv.turns[i];
				break;
			}
		}
...
/** Append a screenshot (and optional UI tree) to a turn. Used for next-step flows where
 * the same turn receives multiple screenshots. Preserves all previous screenshots. */
export function appendScreenshotToTurn(
	clientId: string,
	sessionId: string,
	turnId: number,
	file: string,
	...
	return withWriteLock(key, async () => {
		...
		if (!turn) return;
		const ss = turn.screenshot ?? {};
		const files = Array.isArray(ss.files)
			? [...ss.files]
			: ss.file
				? [ss.file]
				: [];
		const uiTreeFiles = Array.isArray(ss.uiTreeFiles)
			? [...ss.uiTreeFiles]
			: ss.uiTreeFile
				? [ss.uiTreeFile]
				: [];
		files.push(file);
		if (uiTreeFile) uiTreeFiles.push(uiTreeFile);
		deepMerge(turn as unknown as Record<string, unknown>, {
			screenshot: {
				files,
				uiTreeFiles,
				file,
				...(uiTreeFile && { uiTreeFile }),
			},
		});
		await writeConversationRaw(conv);
	});
...
export async function listConversations(clientId: string): Promise<
	Array<{
		id: string;
		createdAt: string;
		updatedAt: string;
...
> {
	...
	try {
		...
		for (const entry of entries) {
			...
			if (conv) {
				const normalizedTurns = dedupeTurnsForRead(conv.turns);
				const firstTurn = normalizedTurns.find((t) => t.input.transcript);
				convs.push({
					id: conv.sessionId,
...
export async function getConversation(
	...
	return {
		...conv,
		turns: dedupeTurnsForRead(conv.turns),
	};
}
